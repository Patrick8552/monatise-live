...
|---|---|---|---|---|---|
| Alpaca | US-equity 15-minute, hourly, and daily OHLC; snapshots; IEX/SIP feed determined by account | No futures market-data API in the current integration | Required stock candle/snapshot authority | `ALPACA_API_KEY` + `ALPACA_API_SECRET` present | Reject stale, future, duplicate, malformed, impossible, or incomplete candles. No unverified fallback. |
| Quiver | Company/ticker alternative data: Congress, insiders, contracts, lobbying, off-exchange activity, and news | Not applicable | Stock alternative-data evidence and conflict checks | `QUIVER_API_KEY` present | Degrade independently when required Alpaca market data remains valid. |
...
| FlashAlpha | US equity/ETF options positioning, gamma exposure, flip, and walls, subject to account tier | Options-on-futures analytics for provider-supported and entitled symbols; official documentation explicitly supports ES/NQ and documents broader CME coverage by tier | Stock confirmation and specialist futures positioning/levels | `FLASHALPHA_API_KEY` present | Validate exact symbol, provider timestamp, freshness, and required values for every futures request; fail closed per instrument. |
...
```text
Alpaca validated 1H + 15M OHLC and snapshot
  + Quiver directional alternative data
  + Finnhub supplemental company context
  + FlashAlpha options-positioning confirmation
