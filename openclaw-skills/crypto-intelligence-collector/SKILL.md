...
Use `~/.openclaw/workspace/tools/monatise-readonly-status SYMBOL INTERVAL`. Never read its token file or print credentials. Record source and timestamps. CoinGlass is the only permitted market-data provider. When it is unavailable, return provider unavailable; do not substitute another provider or invent absent fields.
