...
def run_real_pipeline(direction: str = "long", *, grid: bool = False):
    ...
    inputs = {
        ...
        "macro": MacroRequest("BTC", now),
        "regime": lambda c: RegimeRequest(output(c, "market_data"), output(c, "macro"), trend_threshold=0.5, compression_threshold=0.01, expansion_threshold=100, high_volatility_threshold=200),
...
def decision_request(outputs, **kwargs):
    return DecisionRequest(outputs["market_data"], outputs["macro"], outputs["regime"], outputs["liquidity"], outputs["liquidity_sweep"], outputs["supply_demand"], outputs["reclaim"], outputs["market_structure"], outputs["fibonacci_liquidity"], outputs["order_flow"], require_structure_for_trend=False, require_two_sided_liquidity_for_grid=False, **kwargs)


def test_real_no_trade_and_conditional_decision_paths_block():
    ...
    conditional = DecisionEngine().assess(decision_request(outputs, minimum_conviction=0.7, high_conviction=0.8, maximum_conflict_ratio=1))
    assert conditional.state is DecisionState.CONDITIONAL
...
def test_real_expired_signal_risk_rejection_and_allocation_block():
    ...
    outputs = completed.context.outputs
    now = datetime.now(timezone.utc)
    expired = RiskValidationEngine().assess(RiskRequest(outputs["market_data"], outputs["decision"], outputs["macro"], outputs["regime"], outputs["market_structure"], outputs["fibonacci_liquidity"], outputs["supply_demand"], outputs["order_flow"], outputs["rsi"], now, now - timedelta(seconds=1), proposed_entry=120, proposed_invalidation=117, proposed_target=126, account_equity=100_000))
...
def test_real_market_no_data_and_macro_event_lock_stop_pipeline():
    now = datetime.now(timezone.utc)
    ...
    class EventProvider(MacroProvider):
        def economic_events(self): return [MacroEvent("CPI", now, MacroEventImpact.CRITICAL, currency="USD")]

    macro_app = create_application(market_data_providers={"mock": CandleProvider(now)}, macro_provider=EventProvider())
    locked = asyncio.run(macro_app.orchestrator.run(AnalysisRun("BTC", {"market_data": MarketDataRequest("BTC", interval="1m", candle_limit=200), "macro": MacroRequest("BTC", now)}, metadata=PipelineExecutionMetadata(retry_delay_seconds=0))))
    assert locked.status is PipelineStage.BLOCKED and locked.blocked_by == "macro"
